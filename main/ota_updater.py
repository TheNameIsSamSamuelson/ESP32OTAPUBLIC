...
class OTAUpdater:
    ...
    def __init__(self, github_repo, wifiAuth, module='', main_dir='main'):
        self.wifiAuth = wifiAuth
        self.http_client = HttpClient()
        self.github_repo = github_repo.rstrip('/').replace('https://github.com', 'https://api.github.com/repos')
        self.main_dir = main_dir
        self.module = module.rstrip('/')

    def is_network_avaliable(self):
        host = 'api.github.com'
        port = 443
        import network
        sta_if = network.WLAN(network.STA_IF)
        if not ( sta_if.isconnected() and len(usocket.getaddrinfo(host, port, 0, usocket.SOCK_STREAM)) != 0 ):
            print('connecting to network...')
            sta_if.active(True)
            for auth in self.wifiAuth:
                print("Trying to connect to the wifi : " + auth[0])
                sta_if.connect(auth[0], auth[1])
                if sta_if.isconnected() and len(usocket.getaddrinfo(host, port, 0, usocket.SOCK_STREAM)) != 0:
                    print("Is connected to the wifi " + auth[0] + " and the io of git sorce is: "+ str(usocket.getaddrinfo(host, port, 0, usocket.SOCK_STREAM)[0][-1]))
                    break
                if sta_if.isconnected() and len(usocket.getaddrinfo(host, port, 0, usocket.SOCK_STREAM)) == 0:
                    print("Is connected to the wifi " + auth[0] + " but that wifi is not connected to the internet")

        print('network config:', sta_if.ifconfig())
        import time
        time.sleep_ms(100)
        return len(usocket.getaddrinfo(host, port, 0, usocket.SOCK_STREAM)) != 0


    def check_for_update_to_install_during_next_reboot(self):
        self.is_network_avaliable()
    ...
    def _download_and_install_update(self, latest_version):
        ...
        #Can modify here to make it update the boot.py file maybe
